...
def create_or_append_config(location, name=False):
    ...
    if not os.path.isfile(config_directory + "/" + file_name):
        ...
        # Append to the file
        with open(config_directory + "/" + file_name, "a") as project_config, open(
            location, "r"
        ) as package_config:
...
def append_web_routes(location):
    ...
    with open(routes_file, "a") as project_routes, open(
        location, "r"
    ) as package_routes:
...
def create_controller(location, to="app/http/controllers"):
    ...
    if os.path.isfile(controller_file):
        ...
        shutil.copyfile(location, controller_file)
...
def add_venv_site_packages():
    ...
    if "VIRTUAL_ENV" in os.environ:
        python_version = None
        venv_directory = os.listdir(os.path.join(os.environ["VIRTUAL_ENV"], "lib"))
        ...
        if python_version:
            site_packages_directory = os.path.join(
                os.environ["VIRTUAL_ENV"], "lib", python_version, "site-packages"
            )

            sys.path.append(site_packages_directory)
        else:
            print(
                "\033[93mWARNING: Could not add the virtual environment you are currently in. Attempting to add: {0}\033[93m".format(
